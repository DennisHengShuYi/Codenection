...
export function titleVocabulary({
  ...
  const seen = (rawTitle: string | undefined, at: number): void => {
    ...
    const use = uses.get(key) ?? { label: title, count: 0, lastUsed: at, spellings: new Map() }
  ...
  for (const entry of blockLog) seen(entry.title, entry.answeredAt)
  ...
  schedule.items.forEach((item, index) => seen(item.title, NOW + index))

  return [...uses.values()]
    .map((use) => ({
      ...use,
      label: [...use.spellings.entries()].sort((left, right) => right[1] - left[1])[0]?.[0] ?? use.label,
    }))
