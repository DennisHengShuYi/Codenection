...
*(Amended: the score carries three further terms, all sized as tiebreakers that can never outrank the floor — `deficitArea`, because `min()` saturates at zero once a student has bottomed out and leaves the search no gradient at exactly the moment it matters most; `deadlinePressure` and `neglectPressure` (Ruling 20), because without them deferring work was free and undated work — health, rest, seeing people — always lost; and a **daily-load penalty**, which is the newest. The objective was silent about how full a single day is: §6.6's state multiplier only bites below a reserve of 70, so for a rested student a nine-hour day costs the same per hour as a four-hour one; the fragmentation term counts blocks rather than hours, so moving one block between two days changes nothing, and it rates one nine-hour block better than three short ones; and the daily cap is a wall rather than a slope. Measured on a real fortnight, moving three hours off a nine-hour day onto a one-hour day changed the score by 0.0000 — the solver was not declining to level the week, it could not see the difference. The term charges the square of whole hours past a seven-hour day. Seven and the whole-hour quantisation are both measured rather than chosen: at six, the search-budget fixtures went from 2,520 evaluations to 4,342 against a bound of 3,000, because their days sit right at six hours and every one gained a gradient.)*

**Search: hill climbing with random restarts.** Neighbours are generated by moving one movable task one day, batching two same-type errands into a block, inserting a rest block into a gap, inserting restorative social contact (§5.2 — social reserve is refilled only by contact, so without this move the solver can watch it fall to zero with nothing in its vocabulary to do about it), or **reordering within a day** (§6.6). *(Amended: no neighbour may touch a day already behind the student. The neighbourhood had no notion of "today" at all, so the cheapest improvement available was always to insert recovery into days already lived — the engine re-projects from day zero, so retroactive rest lifts the whole fortnight including the trough `min()` reads. Measured on a real account with today at day 7: all five blocks the solver added landed in the past, and it reported "your worst day goes from 29 to 62" where leaving the past alone gave 29 to 30.)* Best neighbour, repeat to convergence or 200 iterations, three restarts. Roughly fifty lines, under 100ms on a phone. No solver library, no backend call.
